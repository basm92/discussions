...
$$
\text{Coverage Error}(I_{\text{US}}(h)) \approx \underbrace{C_1 \cdot n^{-1}h^{-1}}_{\text{Variance Term}} + \underbrace{C_2 \cdot (nh^{p+1-\nu})^2}_{\text{Squared Bias Term}} + \dots
$$

- To get the best possible US interval, we must choose $h$ to balance these two terms. This leads to the optimal undersmoothing bandwidth, $h_{US}$, which has a rate of $h_{US} \propto n^{-1/(p+2)}$. Plugging this optimal rate back into the error formula shows that the fastest possible coverage error for $I_{US}$ vanishes at a rate of $O(n^{-(p+1)/(p+2)})$.
\normalsize
